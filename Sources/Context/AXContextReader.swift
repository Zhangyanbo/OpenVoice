...
struct InsertionTarget {
    ...
    let requiresPasteInsertion: Bool
    /// 密码框不读取选区，也不调用复制菜单。
    let isSecureTextField: Bool

    init(pid: pid_t, element: AXUIElement?, selectionRange: CFRange?,
         requiresPasteInsertion: Bool = false, isSecureTextField: Bool = false) {
        self.pid = pid
        self.element = element
        self.selectionRange = selectionRange
        self.requiresPasteInsertion = requiresPasteInsertion
